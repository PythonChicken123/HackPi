...
def print_onTFT(text, x_pos, y_pos, text_col): 
    text_area = label.Label(terminalio.FONT, text=text, color=text_col)
    text_group = displayio.Group(scale=FONTSCALE1,x=x_pos,y=y_pos)
    text_group.append(text_area)  # Subgroup for text scaling
    splash.append(text_group)
    
inner_rectangle()
print_onTFT("Welcome to", 30, 40, TEXT_COLOR)
print_onTFT("HackyPi", 60, 80, TEXT_COLOR)
time.sleep(2)
...
draw_rectangle()
print_onTFT("SB COMPONENTS", 20, 30, TEXT_COLOR)
print_onTFT("THANKS FOR BUYING", 20, 60, TEXT_COLOR)
print_onTFT("OUR PRODUCTS....", 20, 90, TEXT_COLOR)
time.sleep(2)